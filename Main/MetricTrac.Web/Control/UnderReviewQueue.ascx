<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="UnderReviewQueue.ascx.cs" Inherits="MetricTrac.Control.UnderReviewQueue" %>
...
<telerik:RadAjaxPanel runat="server" ID="rapFrequency" LoadingPanelID="RadAjaxLoadingPanel1" EnableEmbeddedScripts="true" EnableAJAX="true" OnAjaxRequest="rapFrequency_AjaxRequest" 
        ClientEvents-OnRequestStart="qrapFrequency_RequestStart">
    <telerik:RadWindowManager ID="rwmEditValue" runat="server" Modal="true" VisibleStatusbar="false" Height="525px" Width="775px" DestroyOnClose="true"  style="z-index: 90000"/>
        <div id="Div2" runat="server">
            <script type="text/javascript">
                function qopenRadWindow(url) {
                    var oWnd = radopen(url, "qrwValueEdit");
                    oWnd.Center();
                }
                function qDateNavigation(arg) {
                    var rapFrequency = $find("<%=rapFrequency.ClientID%>");
                    rapFrequency.ajaxRequest(arg);
                }                
                function qrapFrequency_RequestStart() {
                    var p = $find("<%= RadAjaxLoadingPanel1.ClientID %>");
                    p.show("<%= rapFrequency.ClientID %>");
                }
                function qrapFrequency_ResponseEnd() {
                    var p = $find("<%= RadAjaxLoadingPanel1.ClientID %>");
                    p.hide("<%= rapFrequency.ClientID %>");
                }
            </script>
        </div>        
        <mits:NoticeMessageBox ID="lblAlert" runat="server" MessageType="Error" Message="Data Values Under Review" Description="" Width="500px" />
        <div class="metric">
                ...
                <asp:Repeater runat="server" ID="rMetric" OnItemDataBound="rpMetric_ItemDataBound">
                    <ItemTemplate>
                        <tr>
                            ...
                            <th>&nbsp;</th>                            
                            <th class="arrow">                                    
                                <asp:HyperLink runat="server" ID="hlNavL1" NavigateUrl='<%#"javascript:qDateNavigation(\"L1|" + Eval("FrequencyID") + "|" + Eval("MetricID") + "|" + Eval("OrgLocationID") + "|" + Eval("PreviousDate") + "|" + Eval("NextDate") + "|" + Eval("IsNextValues") + "\")"%>' EnableViewState="false">
                            ...
                            <th class="arrow">                                    
                                <asp:HyperLink runat="server" ID="hlNavR1" NavigateUrl='<%#"javascript:qDateNavigation(\"R1|" + Eval("FrequencyID") + "|" + Eval("MetricID") + "|" + Eval("OrgLocationID") + "|" + Eval("PreviousDate") + "|" + Eval("NextDate") + "|" + Eval("IsNextValues") + "\")"%>' EnableViewState="false">
                        ...
                        <tr>
                            <th class="empty">&nbsp;</th>
                            <td class="title">
                                <asp:PlaceHolder ID="PlaceHolder1" runat="server" Visible='<%#(GroupByMetric && DataMode == MetricTrac.Control.MetricInputList.Mode.Input && ((int)Eval("MetricTypeID") == 1) && ((bool)Eval("AllowCustomNames")) && (!String.IsNullOrEmpty((String)Eval("MetricOrgLocationAlias")) || !String.IsNullOrEmpty((String)Eval("MetricOrgLocationCode"))) )%>'>
                                    <div>
                                ...
                                <asp:PlaceHolder ID="PlaceHolder10" runat="server" Visible='<%#!(GroupByMetric && DataMode == MetricTrac.Control.MetricInputList.Mode.Input && ((int)Eval("MetricTypeID") == 1) && ((bool)Eval("AllowCustomNames")) && (!String.IsNullOrEmpty((String)Eval("MetricOrgLocationAlias")) || !String.IsNullOrEmpty((String)Eval("MetricOrgLocationCode"))) )%>'>                                
                                    <%#GroupByMetric ? Eval("Name") : Eval("OrgLocationFullName")%>
                                </asp:PlaceHolder></td>
                            <td class="location" align="right">
                                <asp:PlaceHolder ID="tMetricAlias" runat="server" Visible='<%#(!GroupByMetric && DataMode == MetricTrac.Control.MetricInputList.Mode.Input && ((int)Eval("MetricTypeID") == 1) && ((bool)Eval("AllowCustomNames")) && (!String.IsNullOrEmpty((String)Eval("MetricOrgLocationAlias")) || !String.IsNullOrEmpty((String)Eval("MetricOrgLocationCode"))) )%>'>
                                    <div>
                                ...
                                <asp:PlaceHolder ID="phetricAlias" runat="server" Visible='<%#!(!GroupByMetric && DataMode == MetricTrac.Control.MetricInputList.Mode.Input && ((int)Eval("MetricTypeID") == 1) && ((bool)Eval("AllowCustomNames")) && (!String.IsNullOrEmpty((String)Eval("MetricOrgLocationAlias")) || !String.IsNullOrEmpty((String)Eval("MetricOrgLocationCode"))) )%>'>                                
                                    <%#GroupByMetric ? Eval("OrgLocationFullName") : (((int)Eval("MetricTypeID") == 2 ? "<b>" : "") + Eval("Name"))%>
                                </asp:PlaceHolder></td>
                            <td runat="server" id="tdUnitLeft" class="unit" title='<%#Eval("InputUnitOfMeasureNameTooltip")%>'><%#Eval("InputUnitOfMeasureName")%></td>
                            <asp:Repeater runat="server" ID="rMericValue" >
                                <ItemTemplate>
                                    <td class="cal-metric<%#IsEmptyValue(Container)?" empty":""%>" title='<%#GetValueTitle(Container)%>'><nobr>
                            ...
                            <td runat="server" id="tdUnitRight" class="unit" title='<%#Eval("InputUnitOfMeasureNameTooltip")%>'><%#Eval("UnitOfMeasureName")%></td>
                        </tr>
                    </ItemTemplate>
                </asp:Repeater>
            </ItemTemplate>
