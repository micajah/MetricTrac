<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="MetricValueList.ascx.cs" Inherits="MetricTrac.Control.MetricValueList" %>
                        ...
                        <asp:PlaceHolder ID="PlaceHolder1" runat="server" Visible='<%#IsNewGroup(Container)%>'>
                            ...
                            <tr>
                                ...
                                <th class="title"><%#GroupByMetric?Eval("Name"):Eval("OrgLocationFullName")%>&nbsp;
                                    <asp:PlaceHolder ID="PlaceHolder2" runat="server" Visible='<%#(GroupByMetric && ((int)Eval("MetricTypeID") != 1))%>'><a href='<%#GetRelatedUrl(Container)%>' class="cursor">(Calc&nbsp;Metric)</a></asp:PlaceHolder>
                                ...
                                <th class="arrow">
                                    <asp:HyperLink runat="server" ID="hlNavL1" NavigateUrl='<%#"javascript:DateNavigation(\"L|"+Eval("FrequencyID") + "|" + (GroupByMetric ? Eval("MetricID") : Eval("OrgLocationID")) + "\")"%>' EnableViewState="false">
                                ...
                                <th class="arrow">
                                    <asp:HyperLink runat="server" ID="hlNavR1" NavigateUrl='<%#"javascript:DateNavigation(\"R|"+Eval("FrequencyID") + "|" + (GroupByMetric ? Eval("MetricID") : Eval("OrgLocationID")) +"\")"%>' EnableViewState="false">
                        ...
                        <tr>
                            ...
                            <td colspan="2" class="location" align="right">
                                <asp:PlaceHolder ID="tMetricAlias" runat="server" Visible='<%#(!GroupByMetric && DataMode == Mode.Input && ((int)Eval("MetricTypeID") == 1) && ((bool)Eval("AllowCustomNames")) && (!String.IsNullOrEmpty((String)Eval("MetricOrgLocationAlias")) || !String.IsNullOrEmpty((String)Eval("MetricOrgLocationCode"))) )%>'>
                                    <div>
                                        <asp:PlaceHolder ID="PlaceHolder5" runat="server" Visible='<%#(!String.IsNullOrEmpty((String)Eval("MetricOrgLocationAlias")) )%>'><%#Eval("MetricOrgLocationAlias")%></asp:PlaceHolder>
                                        ...
                                        <asp:PlaceHolder ID="PlaceHolder7" runat="server" Visible='<%#(!String.IsNullOrEmpty((String)Eval("MetricOrgLocationCode")) )%>'><%#Eval("MetricOrgLocationCode")%></asp:PlaceHolder>
                            ...
                            <asp:Repeater runat="server" ID="rMericValue" >
                                <ItemTemplate>
                                    <td class="cal-metric<%#(IsEmptyValue(Container))? ((DataMode == Mode.Input && !IsEditValue(Container)) ? " empty-disabled" : " empty"):""%>" title='<%#GetValueTitle(Container)%>'>                                    
                                        <asp:PlaceHolder ID="PlaceHolder9" runat="server" Visible='<%#IsEditValue(Container)%>'><a title='<%#GetValueTitle(Container)%>' href='<%#GetEditUrl(Container)%>' <%#GetOnClickHandler(Container)%> class="cursor"></asp:PlaceHolder>                                            
                                                <big <%#GetValueCss(Container)%> <%#GetCalcStatusStyle(Container)%>><%#GetValueCell(Container)%></big>
                                        <asp:PlaceHolder ID="PlaceHolder8" runat="server" Visible='<%#IsEditValue(Container)%>'></a></asp:PlaceHolder>
                                    </td>
                                </ItemTemplate>
                            </asp:Repeater>
