...
{    
    public partial class MetricInputList : System.Web.UI.UserControl
    {
        ...
        protected bool GroupByMetric
        {
            get
            {
                return cGroupBy.GroupByMetric;
            }
        }

        protected Bll.GroupByMode GroupBy
        {
            get
            {
                return cGroupBy.SelectedGroupByMode;
            }
        }
        ...
        void ReBind()
        {
            ...
            List<MetricTrac.Bll.FrequencyMetric> fms = MetricTrac.Bll.MetricValue.List(6, CurrentDate, FrequencyFirstDate, SelMetricID,
                SelOrgLocationsID, SelGcaID, SelPiID, SelCollectorUseID, DataMode == Mode.View || DataMode == Mode.Approve, GroupBy, ApproverUserId);
            ...
            if (DataMode == Mode.View && SelPiID != null && SelPiID.Length>0)
            {
                var PiFL = MetricTrac.Bll.PerformanceIndicator.GetValues(CurrentDate, 6, SelPiID, MetricTrac.PerformanceIndicatorCalc.CalcStringFormula);
                foreach (var PiF in PiFL)
                {
                    var ExistFreq = fms.Where(f => f.FrequencyID == PiF.FrequencyID).FirstOrNull();
                    if (ExistFreq == null)
                    {
                        fms.Add(PiF);
                    }
                    ...
                    {
                        ExistFreq.Metrics.InsertRange(0, PiF.Metrics);
                    }
                }
                fms = fms.OrderBy(f => f.FrequencyID).ToList();
                //if (freq != null && freq.Metrics != null && freq.Metrics.Count > 0) fms.Insert(0, freq);
            }
