...
namespace MetricTrac
{
    public partial class MasterDataTree : System.Web.UI.Page
    {
        ...
        List<Bll.ViewHierarchyMetricCategory> MC_Includes = null;
        ...
        {
            PI = Bll.PerformanceIndicator.List();
            ...
            MC_Includes = Bll.MetricCategory.SelectAllIncludes().ToList(); // active mc
            M = Bll.Metric.List().ToList(); // active metrics


            /*Data = Bll.MasterData.GetMasterData().ToList(); // with virtual
            
            JuncPIFPI = null;//Bll.PerformanceIndicatorFormPerformanceIndicatorJunc.PIFPIJuncList().ToList();
            JuncPIFOrgLocation = null;//Bll.Mc_EntityNode.PIFOrgLocationsJuncList().ToList();
            
            PIF = null;//Bll.PerformanceIndicatorForm.List().ToList(); // active pif
            
            
            D_GCA = null;//Bll.GroupCategoryAspect.D_SelectAll().ToList(); // active gca                
            
            D_MC = null;//Bll.MetricCategory.D_SelectAll().ToList(); // active mc
            OrgLocations = Bll.Mc_EntityNode.ListOrgLocations();// active locations
            D_OrgLocations = null;//Bll.Mc_EntityNode.D_ListOrgLocations();// active locations*/
            switch (e.Node.Value)
            {
                case PINodeValue:                    
        ...
        private void AppendMetricsByCategory(Guid PI, RadTreeNode PINode)
        {
            ...
            List<Guid> NecesseryMCID = MC_Includes.Where(dmc => UsedMCID.Contains((Guid)dmc.SubMetricCategoryID)).Select(dmc => (Guid)dmc.MetricCategoryID).Distinct().ToList();
