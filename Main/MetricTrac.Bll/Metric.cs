...
namespace MetricTrac.Bll
{
    ...
    {
        public class MetricOrgLocation
        {
        ...
        public class Extend : Metric
        {
            ...
            public List<MetricOrgLocation> AssignedOrgLocations;
            public string AssignedLocationsNames { get; set; }
        ...
        public static List<Metric> List(Guid? OrgLocationID, Guid? GroupCategoryAspectID, Guid? PerformanceIndicatorID, Guid? CollectorUserId)
        {            
            LinqMicajahDataContext dc = new LinqMicajahDataContext();

            ISingleResult<Sp_SelectMetricListResult> result = null;//dc.Sp_SelectMetricList(LinqMicajahDataContext.InstanceId, CollectorUserId, OrgLocationID, GroupCategoryAspectID, PerformanceIndicatorID);
        ...
        public static List<Metric> List(Guid?[] OrgLocationsID, Guid? GroupCategoryAspectID, Guid?[] PerformanceIndicatorID, Guid? CollectorUserId)
        {
            LinqMicajahDataContext dc = new LinqMicajahDataContext();
            string OrgLocations = MetricTrac.Bll.MetricValue.GetLocationsEncodedString(OrgLocationsID, ',');
            string PerformanceIndicators = MetricTrac.Bll.MetricValue.GetLocationsEncodedString(PerformanceIndicatorID, ',');            
            ISingleResult<Sp_SelectMetricListResult> result = dc.Sp_SelectMetricList(LinqMicajahDataContext.InstanceId, CollectorUserId, GroupCategoryAspectID, OrgLocations, PerformanceIndicators);
            List<Metric> resultList = new List<Metric>();
            foreach (Sp_SelectMetricListResult res in result)
                resultList.Add(CreateMetric(res));
            return resultList.OrderBy(m => m.Name).ToList();
        }

        private static Metric CreateMetric(Sp_SelectMetricListResult result)
        {   
            Metric m = new Metric();
            m.InstanceId = (Guid)result.InstanceId;
            m.MetricID = (Guid)result.MetricID;
            m.Name = result.Name;
            m.Alias = result.Alias;
            m.Code = result.Code;
            m.Status = result.Status;
            m.Created = (DateTime)result.Created;
            m.Updated = result.Updated;
            m.MetricCategoryID = result.MetricCategoryID;
            m.UnitOfMeasureID = result.UnitOfMeasureID;
            m.InputUnitOfMeasureID = result.InputUnitOfMeasureID;
            m.MetricTypeID = (int)result.MetricTypeID;
            m.MetricDataTypeID = (int)result.MetricDataTypeID;
            m.NODecPlaces = result.NODecPlaces;
            m.NOMaxValue = result.NOMaxValue;
            m.NOMinValue = result.NOMinValue;
            m.Definition = result.Definition;
            m.Documentation = result.Documentation;
            m.MetricReferences = result.MetricReferences;
            m.FormulaCode = result.FormulaCode;
            m.Variable = result.Variable;
            m.GrowUpIsGood = result.GrowUpIsGood;
            return m;
        }

        public static List<Extend> ListNumeric(Guid PIID)
        {
            List<Extend> l = List(PIID);
            l=l.Where(m => m.MetricDataTypeID == 1).ToList();
            return l;
        }
        ...
        public static List<Metric.Extend> List(Guid PerformanceIndicatorID)
        {
            ...
            List<Metric.Extend> l = Metrics.ToList();
            return l;
        }
               
        public static List<Sp_SelectUnassignedMetricResult> ListUnused(Guid PerformanceIndicatorID, Guid?[] PIID, Guid? GCAID, Guid?[] OrgLocationsID, Guid? DataCollectorID)
        {
            LinqMicajahDataContext dc = new LinqMicajahDataContext();
            string OrgLocations = MetricTrac.Bll.MetricValue.GetLocationsEncodedString(OrgLocationsID, ',');
            string PerformanceIndicators = MetricTrac.Bll.MetricValue.GetLocationsEncodedString(PIID, ',');
            var rv = dc.Sp_SelectUnassignedMetric(LinqMicajahDataContext.InstanceId, PerformanceIndicatorID, DataCollectorID, OrgLocations, PerformanceIndicators, GCAID);
        ...
        public static List<MetricOrgDataRulesResult> ListRulesResult()
        {
            ...
            var q = from mo in dc.FxGetRule(LinqMicajahDataContext.InstanceId)
                    join m in dc.Metric on new { LinqMicajahDataContext.InstanceId, mo.MetricID } equals new { m.InstanceId, MetricID = (Guid?)m.MetricID }
                    //join o in dc.EntityNodeFullNameView on mo.OrgLocationID equals (Guid?)o.EntityNodeId
                    join o in dc.ViewnameOrgLocation on new { mo.InstanceId, mo.OrgLocationID } equals new { o.InstanceId, o.OrgLocationID }

                    join _cu in dc.ViewnameUser on mo.CollectorUserId equals _cu.UserId into __cu
                    ...
                    join _au in dc.ViewnameUser on mo.ApproverUserId equals _au.UserId into __au
                    ...
                    orderby (o.OrgLocationID == Guid.Empty ? null : o.FullName), mo.OrgLocationID, m.Name, mo.MetricID
                    ...
                    select new MetricOrgDataRulesResult
                    {
                        ...
                        CollectorName = (mo.CollectorUserId == null) ? ("<b>Group:</b> " + cg.Name) : ("<b>User:</b> " + cu.NameEmail),

                        ApproverUserID = mo.ApproverUserId,
                        ApproverGroupID = mo.ApproverGroupId,
                        ApproverName = (mo.ApproverUserId == null) ? ("<b>Group:</b> " + ag.Name) : ("<b>User:</b> " + au.NameEmail),
                    };
            ...
            {
                var GCAs = (
                    from p in dc.ViewPathExtended//D_MetricOrgLocationPath
                    ...
                    join g in dc.ViewnameGroupCategoryAspect on new { p.GroupCategoryAspectID, p.InstanceId } equals new { g.GroupCategoryAspectID, g.InstanceId }
                    ...
                    {
                        GroupCategoryAspectID = (Guid)g.GroupCategoryAspectID,
                        InstanceId = (Guid)p.InstanceId,
                        FullName = g.FullName,
                        IsVirtual = p.IsVirtual == 1
                    }
                ).Distinct().ToList();
        ...
        public static List<Metric.Extend> GetBaseMetrics(Guid? MetricCategoryID, int FrequencyID)
        {
            ...
            var Metrics =
                from m in dc.Metric
                join _d in dc.ViewHierarchyMetricCategory/*D_MetricCategory*/ on
                    new { InstanceId = (Guid?)m.InstanceId, m.MetricCategoryID } equals
                    new { _d.InstanceId, MetricCategoryID = _d.SubMetricCategoryID } into __d
        ...
        // === Page for Metric - Org Location association
        // TODO: move to Org Location class maybe
        public static List<Metric.Extend> ListLocations(Guid OrgLocationID)//Guid.Empty - 
        {
            LinqMicajahDataContext dc = new LinqMicajahDataContext();
            List<ViewPathExtended> Path =
                         (from p in dc.ViewPathExtended
                         join h in dc.ViewHierarchyOrgLocation on new { p.InstanceId, p.OrgLocationID } equals new { h.InstanceId, OrgLocationID = h.SubOrgLocationID }
                         where p.InstanceId == LinqMicajahDataContext.InstanceId && h.OrgLocationID == OrgLocationID
                         select p).ToList();

            List<Metric.Extend> Metrics = (from x in Path
                                          join m in dc.Metric on new { InstanceId = (Guid)x.InstanceId, MetricID = (Guid)x.MetricID } equals new { m.InstanceId, m.MetricID }
                                          select new Extend
                                          {
                                            InstanceId = (Guid)x.InstanceId,
                                            MetricID = (Guid)x.MetricID,
                                            Name = m.Name
                                          }).Distinct().ToList();

            foreach (Metric.Extend m in Metrics)
            {
